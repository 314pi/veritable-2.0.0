 ...
 *              instances in the main-window of vERITABLE
 *
...
{
	static QRegExp s_reVERITABLE_HOSTS( "\\$VERITABLE_HOSTS\\b" );

	QString script = data().toString();

	if ( script.contains( s_reVERITABLE_HOSTS ) )
	{
		QStringList hosts;
		foreach ( Client *cl, _clients )
		{
		...
		script.replace( s_reVERITABLE_HOSTS, hosts.join( " " ) );
	}
	...
	m_classRoomItem( NULL )
{
	...
	m_vncConn = new VeritableVncConnection( this );
	m_vncConn->setHost( m_hostname );
	m_vncConn->setQuality( VeritableVncConnection::ThumbnailQuality );
	...
	m_connection = new VeritableCoreConnection( m_vncConn );
