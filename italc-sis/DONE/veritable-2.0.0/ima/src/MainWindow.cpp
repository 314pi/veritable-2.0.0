				...
				QApplication::desktop()->screenNumber( this ) )
{
	...
	setWindowTitle( tr( "vERITABLE" ) + " " + VERITABLE_VERSION );
						...
						LocalSystem::Path::personalConfigDataPath() ) == false )
	{
		...
		QMessageBox::information( this, tr( "No write access" ),
			...
			"For running vERITABLE, make sure you're permitted to "
...
/*	setCentralWidget( hbox );
	...
	ToolButton * overview_mode = new ToolButton(
			...
			tr( "This is the default mode in vERITABLE and allows you "
...
/*	ToolButton * remotelogon = new ToolButton(
	...
	VeritableVncConnection * conn = new VeritableVncConnection( this );
	// attach VeritableCoreConnection to it so we can send extended vERITABLE commands
	m_localICA = new VeritableCoreConnection( conn );

	conn->setHost( QHostAddress( QHostAddress::LocalHost ).toString() );
	conn->setPort( VeritableCore::config->coreServerPort() );
	...
	if( !conn->waitForConnected( 5000 ) )
	{
		QMessageBox::information( this,
			tr( "Could not contact vERITABLE service" ),
			tr( "Could not contact the local vERITABLE service. It is likely "
				"that you entered wrong credentials or key files are "
				"not set up properly. Try again or contact your "
				"administrator for solving this problem using the vERITABLE "
				"Management Console." ) );
		if( VeritableCore::config->logLevel() < Logger::LogLevelDebug )
		{
			return;
		}
	}
	...
	m_localICA->setRole( VeritableCore::role );
	m_localICA->startDemoServer( VeritableCore::config->coreServerPort(),
									VeritableCore::config->demoServerPort() );

//	##VERITABLE2: m_localISD->hideTrayIcon();
	...
	m_systemTrayIcon.setToolTip( tr( "vERITABLE Master Control" ) );
...
{
	if( VeritableCore::initAuthentication() )
	{
		return true;
	}

	if( VeritableCore::role != VeritableCore::RoleTeacher )
	{
	...
	QMessageBox::information( NULL,
			...
			tr(	"No authentication key files were found or your current ones "
				"are outdated. Please create new key files using the vERITABLE "
				"Management Console. Alternatively set up logon authentication "
				"using the vERITABLE Management Console. Otherwise you won't be "
				"able to access computers using vERITABLE." ) );
...
void MainWindow::aboutVERITABLE( void )
{
