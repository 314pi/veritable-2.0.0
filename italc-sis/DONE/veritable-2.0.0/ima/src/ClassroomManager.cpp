...
ClassroomManager::ClassroomManager( MainWindow * _main_window,
	...
	m_personalConfiguration( LocalSystem::Path::expand( VeritableCore::config->personalConfigurationPath() ) ),
	m_globalClientConfiguration( LocalSystem::Path::expand( VeritableCore::config->globalConfigurationPath() ) ),
...
{
	QDomDocument doc( "veritable-config-file" );

	QDomElement veritable_config = doc.createElement( "globalclientconfig" );
	veritable_config.setAttribute( "version", VERITABLE_VERSION );
	doc.appendChild( veritable_config );

	QDomElement root = doc.createElement( "body" );
	veritable_config.appendChild( root );
...
/*	if( MainWindow::ensureConfigPathExists() == FALSE )
	{
		qFatal( QString( "Could not read/write or create directory %1!"
					"For running vERITABLE, make sure you have "
					"write-access to your home-directory "
					"and to %1 (if already existing)."
					).arg( VERITABLE_CONFIG_PATH
						).toUtf8().constData() );
	}*/
...
void ClassroomManager::savePersonalConfig( void )
{
	...
	QDomElement veritable_config = doc.createElement( "personalconfig" );
	veritable_config.setAttribute( "version", VERITABLE_VERSION );
	doc.appendChild( veritable_config );

	QDomElement head = doc.createElement( "head" );
	veritable_config.appendChild( head );
	...
	globalsettings.setAttribute( "role", VeritableCore::role );
...
mainWindow()->move( node.toElement().attribute( "win-x" ).toInt(),
	...
	mainWindow()->setWindowState( mainWindow()->windowState() |
			...
			VeritableCore::role = static_cast<VeritableCore::UserRoles>(
				node.toElement().attribute( "role" ).toInt() );
			if( VeritableCore::role <= VeritableCore::RoleNone ||
				VeritableCore::role >= VeritableCore::RoleCount )
			{
