/*
 * VeritableCoreConnection.h - declaration of class VeritableCoreConnection
 *
...
#ifndef _VERITABLE_CORE_CONNECTION_H
#define _VERITABLE_CORE_CONNECTION_H

#include "VeritableCore.h"
#include "VeritableVncConnection.h"


class VeritableCoreConnection : public QObject
{
	Q_OBJECT
public:
	VeritableCoreConnection( VeritableVncConnection *vncConnection );
	~VeritableCoreConnection();

	VeritableVncConnection *vncConnection()
	{
		return m_vncConn;
	}

	VeritableVncConnection::State state() const
	{
...
#define GEN_SLAVE_STATE_HELPER(x)							\
			bool is##x() const								\
			{												\
				return slaveStateFlags() & VeritableCore::x;	\
			}
	...
	void setRole( const VeritableCore::UserRole role );
...
private:
	static rfbBool handleVeritableMessage( rfbClient *cl,
						rfbServerToClientMsg *msg );

	bool handleServerMessage( rfbClient *cl, uint8_t msg );
	void enqueueMessage( const VeritableCore::Msg &msg );


	VeritableVncConnection *m_vncConn;
