/*
 * VeritableVncConnection.cpp - implementation of VeritableVncConnection class
 *
...
rfbBool VeritableVncConnection::hookNewClient( rfbClient *cl )
{
	VeritableVncConnection * t = (VeritableVncConnection *)
...
void VeritableVncConnection::hookUpdateFB( rfbClient *cl, int x, int y, int w, int h )
{
	...
	VeritableVncConnection * t = (VeritableVncConnection *) rfbClientGetClientData( cl, 0 );
...
void VeritableVncConnection::hookFinishFrameBufferUpdate( rfbClient *cl )
{
	VeritableVncConnection *t = (VeritableVncConnection *)
					rfbClientGetClientData( cl, 0 );
...
rfbBool VeritableVncConnection::hookHandleCursorPos( rfbClient *cl, int x, int y )
{
	VeritableVncConnection * t = (VeritableVncConnection *) rfbClientGetClientData( cl, 0 );
	if( t )
	{
...
void VeritableVncConnection::hookCursorShape( rfbClient *cl, int xh, int yh,
											int w, int h, int bpp )
{
	for( int i = 0; i < w*h;++i )
	{
...
void VeritableVncConnection::hookCutText( rfbClient *cl, const char *text,
										int textlen )
{
...
void VeritableVncConnection::hookOutputHandler( const char *format, ... )
{
	va_list args;
	...
	ilog( Warning, "VeritableVncConnection: " + message );
...
VeritableVncConnection::VeritableVncConnection( QObject *parent ) :
	QThread( parent ),
	m_frameBuffer( NULL ),
	m_framebufferInitialized( false ),
	m_cl( NULL ),
	m_veritableAuthType( VeritableAuthDSA ),
...
VeritableVncConnection::~VeritableVncConnection()
{
	stop();
...
void VeritableVncConnection::checkOutputErrorMessage()
{
...
void VeritableVncConnection::stop()
{
	if( isRunning() )
	{
		m_stopped = true;
		...
		if( !wait( 1000 ) )
		{
			qWarning( "VeritableVncConnection::stop(): terminating thread" );
			terminate();
		}
	}
}
...
void VeritableVncConnection::reset( const QString &host )
{
...
void VeritableVncConnection::setHost( const QString &host )
{
...
void VeritableVncConnection::setPort( int port )
{
	QMutexLocker locker( &m_mutex );
	m_port = port;
}
...
bool VeritableVncConnection::waitForConnected( int timeout ) const
{
	QTime startTime;
	startTime.start();
...
void VeritableVncConnection::setImage( const QImage & img )
{
...
const QImage VeritableVncConnection::image( int x, int y, int w, int h ) const
{
...
void VeritableVncConnection::setFramebufferUpdateInterval( int interval )
{
...
void VeritableVncConnection::rescaleScreen()
{
...
void VeritableVncConnection::run()
{
	m_state = Disconnected;
	m_stopped = false;
...
void VeritableVncConnection::doConnection()
{
	QMutex sleeperMutex;
...
void VeritableVncConnection::enqueueEvent( ClientEvent *e )
{
...
void VeritableVncConnection::mouseEvent( int x, int y, int buttonMask )
{
...
void VeritableVncConnection::keyEvent( unsigned int key, bool pressed )
{
...
void VeritableVncConnection::clientCut( const QString &text )
{
...
void VeritableVncConnection::handleSecTypeVeritable( rfbClient *client )
{
	...
	int chosenAuthType = VeritableAuthCommonSecret;
	if( !supportedAuthTypes.isEmpty() )
	{
		chosenAuthType = supportedAuthTypes.values().first().toInt();

		// look whether the VeritableVncConnection recommends a specific
		// authentication type (e.g. VeritableAuthHostBased when running as
		// demo client)
		VeritableVncConnection *t = (VeritableVncConnection *)
		...
		if( t != NULL )
		{
			...
			{
				if( t->veritableAuthType() == v.toInt() )
				{
	...
	{
		socketDev.write( VeritableCore::authenticationCredentials->logonUsername() );
	}
	else
	{
	...
	if( chosenAuthType == VeritableAuthDSA )
	{
		if( VeritableCore::authenticationCredentials->hasCredentials(
				AuthenticationCredentials::PrivateKey ) )
		{
			QByteArray chall = socketDev.read().toByteArray();
			socketDev.write( QVariant( (int) VeritableCore::role ) );
			socketDev.write( VeritableCore::authenticationCredentials->
													privateKey()->sign( chall ) );
		}
	}
	else if( chosenAuthType == VeritableAuthHostBased )
	{
		// nothing to do - we just get accepted if our IP is in the list of
		// allowed hosts
	}
	else if( chosenAuthType == VeritableAuthCommonSecret )
	{
		socketDev.write( VeritableCore::authenticationCredentials->commonSecret() );
	}
	else if( chosenAuthType == VeritableAuthNone )
	{
...
void VeritableVncConnection::handleMsLogonIIAuth( rfbClient *client )
{
	...
	strcpy( user, VeritableCore::authenticationCredentials->
										logonUsername().toUtf8().constData() );
	strcpy( passwd, VeritableCore::authenticationCredentials->
...
{
	if( VeritableCore::config->isLogonAuthenticationEnabled() &&
			VeritableCore::authenticationCredentials->hasCredentials(
										AuthenticationCredentials::UserLogon ) )
	{
...
{
	VeritableVncConnection::handleSecTypeVeritable( client );
}


void handleMsLogonIIAuth( rfbClient *client )
{
	VeritableVncConnection::handleMsLogonIIAuth( client );
}
