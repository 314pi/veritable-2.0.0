 ...
 *              instances in the main-window of iSCHOOL
 *
...
{
	static QRegExp s_reISCHOOL_HOSTS( "\\$ISCHOOL_HOSTS\\b" );

	QString script = data().toString();

	if ( script.contains( s_reISCHOOL_HOSTS ) )
	{
		QStringList hosts;
		foreach ( Client *cl, _clients )
		{
		...
		script.replace( s_reISCHOOL_HOSTS, hosts.join( " " ) );
	}
	...
	m_classRoomItem( NULL )
{
	...
	m_vncConn = new IschoolVncConnection( this );
	m_vncConn->setHost( m_hostname );
	m_vncConn->setQuality( IschoolVncConnection::ThumbnailQuality );
	...
	m_connection = new IschoolCoreConnection( m_vncConn );
		...
		switch( m_mode = _new_mode )
		{
			...
			case Mode_WindowDemo:
				...
				m_connection->startDemo(
								...
								IschoolCore::config->demoServerPort(),
