...
ClassroomManager::ClassroomManager( MainWindow * _main_window,
	...
	m_personalConfiguration( LocalSystem::Path::expand( IschoolCore::config->personalConfigurationPath() ) ),
	m_globalClientConfiguration( LocalSystem::Path::expand( IschoolCore::config->globalConfigurationPath() ) ),
...
{
	QDomDocument doc( "ischool-config-file" );

	QDomElement ischool_config = doc.createElement( "globalclientconfig" );
	ischool_config.setAttribute( "version", ISCHOOL_VERSION );
	doc.appendChild( ischool_config );

	QDomElement root = doc.createElement( "body" );
	ischool_config.appendChild( root );
...
void ClassroomManager::savePersonalConfig( void )
{
	...
	QDomElement ischool_config = doc.createElement( "personalconfig" );
	ischool_config.setAttribute( "version", ISCHOOL_VERSION );
	doc.appendChild( ischool_config );

	QDomElement head = doc.createElement( "head" );
	ischool_config.appendChild( head );
	...
	globalsettings.setAttribute( "role", IschoolCore::role );
...
mainWindow()->move( node.toElement().attribute( "win-x" ).toInt(),
	...
	mainWindow()->setWindowState( mainWindow()->windowState() |
			...
			IschoolCore::role = static_cast<IschoolCore::UserRoles>(
				node.toElement().attribute( "role" ).toInt() );
			if( IschoolCore::role <= IschoolCore::RoleNone ||
				IschoolCore::role >= IschoolCore::RoleCount )
			{
