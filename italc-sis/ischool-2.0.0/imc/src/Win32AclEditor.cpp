...
STDMETHODIMP IschoolSecurityInfo::QueryInterface( REFIID iid, void** ppv )
{
...
STDMETHODIMP_(ULONG) IschoolSecurityInfo::AddRef()
{
	return ++m_cRefs;
}
STDMETHODIMP_(ULONG) IschoolSecurityInfo::Release()
{
	ULONG n = --m_cRefs;
	if ( 0 == n )
		delete this;
...
STDMETHODIMP
IschoolSecurityInfo::GetObjectInformation( SI_OBJECT_INFO* poi ){
...
STDMETHODIMP
IschoolSecurityInfo::GetSecurity(SECURITY_INFORMATION ri,
...
STDMETHODIMP
IschoolSecurityInfo::SetSecurity(SECURITY_INFORMATION ri, void* psd){
	vncAccessControl vncAC;
	return vncAC.SetSD((PSECURITY_DESCRIPTOR) psd) ? S_OK : E_FAIL;
}

STDMETHODIMP
IschoolSecurityInfo::PropertySheetPageCallback(HWND hwnd, UINT msg, SI_PAGE_TYPE pt){
...
STDMETHODIMP
IschoolSecurityInfo::GetAccessRights(const GUID*,
								 DWORD dwFlags,
								 SI_ACCESS** ppAccess,
								 ULONG* pcAccesses,
...
STDMETHODIMP
IschoolSecurityInfo::MapGeneric(const GUID*, UCHAR* pAceFlags, ACCESS_MASK* pMask){
...
STDMETHODIMP
IschoolSecurityInfo::GetInheritTypes(SI_INHERIT_TYPE** ppInheritTypes, ULONG* pcInheritTypes){
...
void Win32AclEditor( HWND hwnd )
{
	...
	IschoolSecurityInfo* psi = new IschoolSecurityInfo(L"iSCHOOL Server", L"iSCHOOL Server");
	psi->AddRef();
	...
	psh.hwndParent	= hwnd;
	psh.pszCaption	= _T("iSCHOOL Logon Authentication");
