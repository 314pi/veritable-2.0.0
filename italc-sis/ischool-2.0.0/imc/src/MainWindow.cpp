...
#define CONNECT_BUTTON_SLOT(name) \
	...
	connect( IschoolCore::config, SIGNAL( configurationChanged() ),
				this, SLOT( configurationChanged() ) );

#ifndef ISCHOOL_BUILD_WIN32
...
void MainWindow::reset( bool onlyUI )
{
	if( onlyUI == false )
	{
		IschoolCore::config->clear();
		*IschoolCore::config += IschoolConfiguration::defaultConfiguration();
		*IschoolCore::config += IschoolConfiguration( Configuration::Store::LocalBackend );
	}
	...
	FOREACH_ISCHOOL_CONFIG_PROPERTY(INIT_WIDGET_FROM_PROPERTY)
...
{
#ifdef ISCHOOL_BUILD_WIN32
	IschoolCore::config->setValue( "EncodedLogonACL", LogonAclSettings().acl(),
															"Authentication" );
#endif
	if( ImcCore::applyConfiguration( *IschoolCore::config ) )
	{
#ifdef ISCHOOL_BUILD_WIN32
		if( isServiceRunning() &&
			QMessageBox::question( this, tr( "Restart iSCHOOL Service" ),
				tr( "All settings were saved successfully. In order to take "
					"effect the iSCHOOL service needs to be restarted. "
...
{
	serviceControlWithProgressBar( tr( "Starting iSCHOOL service" ), "-startservice" );
}
...
{
	serviceControlWithProgressBar( tr( "Stopping iSCHOOL service" ), "-stopservice" );
}
...
void MainWindow::clearLogFiles()
{
	...
	if( isServiceRunning() )
	{
		if( QMessageBox::question( this, tr( "iSCHOOL Service" ),
				tr( "The iSCHOOL service needs to be stopped temporarily "
	...
	bool success = true;
	QDir d( LocalSystem::Path::expand( IschoolCore::config->logFileDirectory() ) );
	foreach( const QString &f, d.entryList( QStringList() << "Ischool*.log" ) )
	{
		if( f != "IschoolManagementConsole.log" )
		{
			success &= d.remove( f );
		}
	}
...
void MainWindow::loadSettingsFromFile()
{
	...
	if( !fileName.isEmpty() )
	{
		...
		Configuration::XmlStore( Configuration::XmlStore::System,
										fileName ).load( IschoolCore::config );
		reset( true );
...
#ifdef ISCHOOL_BUILD_WIN32
		IschoolCore::config->removeValue( "LogonACL", "Authentication" );
		IschoolCore::config->setValue( "EncodedLogonACL",
								LogonAclSettings().acl(), "Authentication" );
#endif

		// write current configuration to output file
		Configuration::XmlStore( Configuration::XmlStore::System,
										fileName ).flush( IschoolCore::config );
...
void MainWindow::generateBugReportArchive()
{
	...
	QString outfile = fsb.exec( QDir::homePath(),
								...
								tr( "iSCHOOL bug report archive (*.ibra.xml)" ) );
	if( outfile.isEmpty() )
	{
		return;
	}
...
#elif defined( ISCHOOL_BUILD_LINUX )
...
#ifdef ISCHOOL_HOST_X86
	const QString buildType = "x86";
#elif defined( ISCHOOL_HOST_X86_64 )
	...
	obj.setValue( "Version", ISCHOOL_VERSION, "General" );


	// add current iSCHOOL configuration
	obj.addSubObject( IschoolCore::config, "Configuration" );


	// compress all log files and encode them as base64
	QStringList paths;
	paths << LocalSystem::Path::expand( IschoolCore::config->logFileDirectory() );
#ifdef ISCHOOL_BUILD_WIN32
	...
	QMessageBox::information( this, tr( "iSCHOOL bug report archive saved" ),
			tr( "An iSCHOOL bug report archive has been saved to %1. "
				"It includes iSCHOOL log files and information about your "
