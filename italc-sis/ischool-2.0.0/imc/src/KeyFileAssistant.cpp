	...
	m_ui( new Ui::KeyFileAssistant )
{
	...
	QDir d( LocalSystem::Path::expand( IschoolCore::config->privateKeyBaseDir() ) );
	d.cdUp();
...
{
	IschoolCore::UserRole role =
			static_cast<IschoolCore::UserRole>(
					m_ui->userRole->currentIndex() + IschoolCore::RoleTeacher );
