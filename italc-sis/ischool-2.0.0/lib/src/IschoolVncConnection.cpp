/*
 * IschoolVncConnection.cpp - implementation of IschoolVncConnection class
 *
...
rfbBool IschoolVncConnection::hookNewClient( rfbClient *cl )
{
	IschoolVncConnection * t = (IschoolVncConnection *)
...
void IschoolVncConnection::hookUpdateFB( rfbClient *cl, int x, int y, int w, int h )
{
	...
	IschoolVncConnection * t = (IschoolVncConnection *) rfbClientGetClientData( cl, 0 );
...
void IschoolVncConnection::hookFinishFrameBufferUpdate( rfbClient *cl )
{
	IschoolVncConnection *t = (IschoolVncConnection *)
					rfbClientGetClientData( cl, 0 );
...
rfbBool IschoolVncConnection::hookHandleCursorPos( rfbClient *cl, int x, int y )
{
	IschoolVncConnection * t = (IschoolVncConnection *) rfbClientGetClientData( cl, 0 );
	if( t )
	{
...
void IschoolVncConnection::hookCursorShape( rfbClient *cl, int xh, int yh,
											int w, int h, int bpp )
{
	for( int i = 0; i < w*h;++i )
	{
...
void IschoolVncConnection::hookCutText( rfbClient *cl, const char *text,
										int textlen )
{
...
void IschoolVncConnection::hookOutputHandler( const char *format, ... )
{
	va_list args;
	...
	ilog( Warning, "IschoolVncConnection: " + message );
...
IschoolVncConnection::IschoolVncConnection( QObject *parent ) :
	QThread( parent ),
	m_frameBuffer( NULL ),
	m_framebufferInitialized( false ),
	m_cl( NULL ),
	m_ischoolAuthType( IschoolAuthDSA ),
...
IschoolVncConnection::~IschoolVncConnection()
{
	stop();
...
void IschoolVncConnection::checkOutputErrorMessage()
{
...
void IschoolVncConnection::stop()
{
	if( isRunning() )
	{
		m_stopped = true;
		...
		if( !wait( 1000 ) )
		{
			qWarning( "IschoolVncConnection::stop(): terminating thread" );
			terminate();
		}
	}
}
...
void IschoolVncConnection::reset( const QString &host )
{
...
void IschoolVncConnection::setHost( const QString &host )
{
...
void IschoolVncConnection::setPort( int port )
{
	QMutexLocker locker( &m_mutex );
	m_port = port;
}
...
bool IschoolVncConnection::waitForConnected( int timeout ) const
{
	QTime startTime;
	startTime.start();
...
void IschoolVncConnection::setImage( const QImage & img )
{
...
const QImage IschoolVncConnection::image( int x, int y, int w, int h ) const
{
...
void IschoolVncConnection::setFramebufferUpdateInterval( int interval )
{
...
void IschoolVncConnection::rescaleScreen()
{
...
void IschoolVncConnection::doConnection()
{
	QMutex sleeperMutex;
...
void IschoolVncConnection::enqueueEvent( ClientEvent *e )
{
...
void IschoolVncConnection::mouseEvent( int x, int y, int buttonMask )
{
...
void IschoolVncConnection::keyEvent( unsigned int key, bool pressed )
{
...
void IschoolVncConnection::clientCut( const QString &text )
{
...
void IschoolVncConnection::handleSecTypeIschool( rfbClient *client )
{
	...
	int chosenAuthType = IschoolAuthCommonSecret;
	if( !supportedAuthTypes.isEmpty() )
	{
		chosenAuthType = supportedAuthTypes.values().first().toInt();

		// look whether the IschoolVncConnection recommends a specific
		// authentication type (e.g. IschoolAuthHostBased when running as
		// demo client)
		IschoolVncConnection *t = (IschoolVncConnection *)
	...
	{
		socketDev.write( IschoolCore::authenticationCredentials->logonUsername() );
	}
	else
	{
	...
	if( chosenAuthType == IschoolAuthDSA )
	{
		if( IschoolCore::authenticationCredentials->hasCredentials(
				AuthenticationCredentials::PrivateKey ) )
		{
			QByteArray chall = socketDev.read().toByteArray();
			socketDev.write( QVariant( (int) IschoolCore::role ) );
			socketDev.write( IschoolCore::authenticationCredentials->
													privateKey()->sign( chall ) );
		}
	}
	else if( chosenAuthType == IschoolAuthHostBased )
	{
		// nothing to do - we just get accepted if our IP is in the list of
		// allowed hosts
	}
	else if( chosenAuthType == IschoolAuthCommonSecret )
	{
		socketDev.write( IschoolCore::authenticationCredentials->commonSecret() );
	}
	else if( chosenAuthType == IschoolAuthNone )
	{
...
void IschoolVncConnection::handleMsLogonIIAuth( rfbClient *client )
{
	...
	strcpy( user, IschoolCore::authenticationCredentials->
										logonUsername().toUtf8().constData() );
	strcpy( passwd, IschoolCore::authenticationCredentials->
...
{
	if( IschoolCore::config->isLogonAuthenticationEnabled() &&
			IschoolCore::authenticationCredentials->hasCredentials(
										AuthenticationCredentials::UserLogon ) )
	{
...
{
	IschoolVncConnection::handleSecTypeIschool( client );
}


void handleMsLogonIIAuth( rfbClient *client )
{
	IschoolVncConnection::handleMsLogonIIAuth( client );
}
