	...
	m_logFile( NULL )
{
	...
	int ll = IschoolCore::config->logLevel();
...
#ifdef ISCHOOL_BUILD_WIN32
	if( IschoolCore::config->logToWindowsEventLog() )
	{
...
{
	QString logPath = LocalSystem::Path::expand( IschoolCore::config->logFileDirectory() );
...
void Logger::outputMessage( const QString &msg )
{
	...
	if( !IschoolCore::config || IschoolCore::config->logToStdErr() )
	{
