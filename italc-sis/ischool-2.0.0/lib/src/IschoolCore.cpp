/*
 * IschoolCore.cpp - implementation of iSCHOOL Core
 *
...
IschoolConfiguration *IschoolCore::config = NULL;
AuthenticationCredentials *IschoolCore::authenticationCredentials = NULL;

int IschoolCore::serverPort = PortOffsetVncServer;
IschoolCore::UserRoles IschoolCore::role = IschoolCore::RoleOther;


void initResources()
{
	Q_INIT_RESOURCE(IschoolCore);
...
bool IschoolCore::init()
{
	...
	QCoreApplication::setOrganizationName( "iSCHOOL Solutions" );
	QCoreApplication::setOrganizationDomain( "ischoolsolutions.org" );
	QCoreApplication::setApplicationName( "iSCHOOL" );
	...
	config = new IschoolConfiguration( IschoolConfiguration::defaultConfiguration() );
	*config += IschoolConfiguration( Configuration::Store::LocalBackend );
...
bool IschoolCore::initAuthentication( int credentialTypes )
{
	...
	{
		const QString privKeyFile = LocalSystem::Path::privateKeyPath( IschoolCore::role );
		qDebug() << "Loading private key" << privKeyFile << "for role" << IschoolCore::role;
...
QString IschoolCore::userRoleName( UserRole role )
{
...
bool Msg::send()
{
	...
	d << (uint8_t) rfbIschoolCoreRequest;
	d << m_cmd;
	d << m_args;
