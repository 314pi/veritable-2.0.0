	...
	m_sysKeyTrapper( new SystemKeyTrapper( false ) )
{
	...
	if( m_mode == DemoMode )
	{
		m_vncConn.setQuality( IschoolVncConnection::DemoClientQuality );
		m_vncConn.setIschoolAuthType( IschoolAuthHostBased );
	...
	{
		m_vncConn.setQuality( IschoolVncConnection::RemoteControlQuality );
	}
