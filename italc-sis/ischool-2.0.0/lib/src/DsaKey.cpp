 ...
 * the code of its release of iSCHOOL with the OpenSSL project's "OpenSSL" library
					...
					const QByteArray & _sig ) const
{
	...
	if( strcmp( "ischool-dss", ktype ) != 0 && strcmp( "ssh-dss", ktype ) != 0)
	{
...
QByteArray PrivateDSAKey::sign( const QByteArray & _data ) const
{
	...
	Buffer b;
	buffer_init( &b ) ;
	buffer_put_cstring( &b, "ischool-dss" );
...
DSA * keyFromBlob( const QByteArray & _ba )
{
	...
	if( strcmp(ktype, "dsa") == 0 || strcmp(ktype, "ischool-dss" ) == 0 || strcmp(ktype, "ssh-dss" ) == 0 )
	{
		dsa = createNewDSA();
...
bool PublicDSAKey::load( const QString & _file, QString )
{
	...
	while( !( line = ts.readLine() ).isNull() )
	{
		...
		if( line[0] != '#' )
		{
			if( line.section( ' ', 0, 0 ) != "ischool-dss" && line.section( ' ', 0, 0 ) != "ssh-dss")
			{
...
bool PublicDSAKey::save( const QString & _file, QString ) const
{
	...
	QTextStream( &outfile ) << QString( "ischool-dss %1" ).
