...
#ifdef ISCHOOL_HAVE_PWD_H
#include <pwd.h>
#endif

#ifdef ISCHOOL_HAVE_SYS_SOCKET_H
#include <sys/socket.h>
#endif

#ifdef ISCHOOL_HAVE_ARPA_INET_H
#include <arpa/inet.h>
#endif

#ifdef ISCHOOL_HAVE_NETINET_IN_H
#include <netinet/in.h>
#endif

#ifdef ISCHOOL_HAVE_ERRNO_H
#include <errno.h>
#endif
...
#ifdef ISCHOOL_BUILD_WIN32
		LocalSystem::windowsConfigPath( CSIDL_APPDATA ) + QDir::separator() + "iSCHOOL";
#else
		QDir::homePath() + QDir::separator() + ".ischool";
#endif
...
QString Path::privateKeyPath( IschoolCore::UserRoles role, QString baseDir )
{
	if( baseDir.isEmpty() )
	{
		baseDir = expand( IschoolCore::config->privateKeyBaseDir() );
	}
	else
	{
		baseDir += "/private";
	}
	QString d = baseDir + QDir::separator() + IschoolCore::userRoleName( role ) +
					QDir::separator() + "key";
	return QDTNS( d );
}
...
QString Path::publicKeyPath( IschoolCore::UserRoles role, QString baseDir )
{
	if( baseDir.isEmpty() )
	{
		baseDir = expand( IschoolCore::config->publicKeyBaseDir() );
	}
	else
	{
		baseDir += "/public";
...
{
#ifdef ISCHOOL_BUILD_WIN32
	return QDTNS( windowsConfigPath( CSIDL_COMMON_APPDATA ) + QDir::separator() + "iSCHOOL/" );
#else
	return "/etc/ischool/";
#endif
}
