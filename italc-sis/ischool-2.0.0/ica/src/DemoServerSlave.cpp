...
protected:
	virtual void run()
	{
		IschoolVncServer::runVncReflector( m_srcPort, m_dstPort );
	}
...
{
	if( m.cmd() == IschoolSlaveManager::DemoServer::StartDemoServer )
	{
		IschoolCore::authenticationCredentials->setCommonSecret(
						m.arg( IschoolSlaveManager::DemoServer::CommonSecret ) );

		const int srcPort =
				m.argV( IschoolSlaveManager::DemoServer::SourcePort ).toInt();
		const int dstPort =
				m.argV( IschoolSlaveManager::DemoServer::DestinationPort ).toInt();
		if( IschoolCore::config->demoServerBackend() == 0 )
		{
	...
	else if( m.cmd() == IschoolSlaveManager::DemoServer::UpdateAllowedHosts )
	{
		const QStringList allowedHosts =
			m.argV( IschoolSlaveManager::DemoServer::AllowedHosts ).toStringList();
