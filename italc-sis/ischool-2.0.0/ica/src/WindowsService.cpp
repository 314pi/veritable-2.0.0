...
class IschoolServiceSubProcess
{
public:
	IschoolServiceSubProcess() :
...
bool WindowsService::evalArgs( int &argc, char **argv )
{
	...
	{
		IschoolCore::init();
		Logger l( "IschoolServiceMonitor" );
		return runAsService();
	}
	...
	Logger l( "IschoolServiceControl" );
...
{
	IschoolServiceSubProcess ischoolProcess;
	...
	while( WaitForSingleObject( s_stopServiceEvent, 1000 ) == WAIT_TIMEOUT )
	{
		...
		if( oldSessionId != sessionId || sessionChanged )
		{
			...
			if( oldSessionId != SESSION_INVALID || sessionChanged )
			{
				...
				while( lastServiceStart.elapsed() < 10000 && ischoolProcess.isRunning() );
			...
			{
				ischoolProcess.start( sessionId );
				lastServiceStart.restart();
			}

			oldSessionId = sessionId;
		}
		else if( ischoolProcess.isRunning() == false )
		{
			ischoolProcess.start( sessionId );
			oldSessionId = sessionId;
			lastServiceStart.restart();
