/*
 * IschoolVncServer.cpp - implementation of IschoolVncServer, a VNC-server-
...
static rfbBool ischoolCoreNewClient( struct _rfbClientRec *, void * * )
{
	return true;
}
...
static rfbBool lvs_ischoolHandleMessage( struct _rfbClientRec *client,
					void *data,
					const rfbClientToServerMsg *message )
{
	if( message->type == rfbIschoolCoreRequest )
	{
		return IschoolCoreServer::instance()->
				handleIschoolClientMessage( libvncServerDispatcher, client );
	}
	return false;
}
...
static void lvs_ischoolSecurityHandler( struct _rfbClientRec *cl )
{
	bool authOK = IschoolCoreServer::instance()->
								authSecTypeIschool( libvncServerDispatcher, cl );
...
IschoolVncServer::IschoolVncServer() :
	QThread(),
	m_port( IschoolCore::serverPort )
{
}
...
static void runX11vnc( QStringList cmdline, int port, bool plainVnc )
{
	...
	if( !plainVnc && IschoolCore::config->isHttpServerEnabled() )
	{
		QDir d( QCoreApplication::applicationDirPath() );
		if( d.cdUp() && d.cd( "share" ) && d.cd( "ischool" ) &&
				d.cd( "JavaViewer" ) )
		{
			cmdline << "-httpport"
					<< QString::number( IschoolCore::config->httpServerPort() )
		...
		else
		{
			qWarning( "Could not find JavaViewer files. "
						"Check your iSCHOOL installation!" );
		}
	}
	...
	if( plainVnc == false )
	{
		// register iSCHOOL protocol extension
		rfbProtocolExtension pe;
		pe.newClient = ischoolCoreNewClient;
		pe.init = NULL;
		pe.enablePseudoEncoding = NULL;
		pe.pseudoEncodings = NULL;
		pe.handleMessage = lvs_ischoolHandleMessage;
		pe.close = NULL;
		pe.usage = NULL;
	...
	// register handler for iSCHOOL's security-type
	rfbSecurityHandler shi = { rfbSecTypeIschool, lvs_ischoolSecurityHandler, NULL };
...
void IschoolVncServer::runVncReflector( int srcPort, int dstPort )
{
#ifdef ISCHOOL_BUILD_WIN32
	...
	if( IschoolCore::config->isDemoServerMultithreaded() )
	{
		args << "-threads";
	}
