...
{
	if( m.cmd() == IschoolSlaveManager::AccessDialog::Ask )
	{
		QString user = m.arg( IschoolSlaveManager::AccessDialog::User );
		QString host = m.arg( IschoolSlaveManager::AccessDialog::Host );
		int choiceFlags = m.argV( IschoolSlaveManager::AccessDialog::ChoiceFlags ).toInt();

		choiceFlags = exec( user, host, choiceFlags );

		Ipc::Msg( IschoolSlaveManager::AccessDialog::ReportChoice ).
			addArg( IschoolSlaveManager::AccessDialog::ChoiceFlags, choiceFlags ).
				send( this );
