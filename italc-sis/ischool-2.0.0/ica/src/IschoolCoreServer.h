/*
 * IschoolCoreServer.h - IschoolCoreServer
 *
...
#ifndef _ISCHOOL_CORE_SERVER_H
#define _ISCHOOL_CORE_SERVER_H
...
class IschoolCoreServer : public QObject
{
	Q_OBJECT
public:
	IschoolCoreServer();
	virtual ~IschoolCoreServer();

	static IschoolCoreServer *instance()
	{
		Q_ASSERT( _this != NULL );
		return _this;
	}

	int handleIschoolClientMessage( socketDispatcher sd, void *user );

	bool authSecTypeIschool( socketDispatcher sd, void *user );

	IschoolSlaveManager * slaveManager()
	{
...
private:
	...
	static IschoolCoreServer *_this;
	...
	IschoolSlaveManager m_slaveManager;
