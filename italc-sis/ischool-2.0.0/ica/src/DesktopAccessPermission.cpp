...
bool DesktopAccessPermission::ask( const QString &user, const QString &host )
{
	...
	switch( m_authenticationMethod )
	{
		case KeyAuthentication:
			if( !IschoolCore::config->isPermissionRequiredWithKeyAuthentication() )
			{
				return true;
			}
			break;
		case LogonAuthentication:
			if( !IschoolCore::config->isPermissionRequiredWithLogonAuthentication() )
			{
				return true;
			}
			if( IschoolCore::config->isSameUserConfirmationDisabled() &&
					!user.isEmpty() &&
	...
	int r = IschoolCoreServer::instance()->slaveManager()->
