	...
	m_vncConn()
{
	...
	m_vncConn.setIschoolAuthType( IschoolAuthCommonSecret );
	m_vncConn.setQuality( IschoolVncConnection::DemoServerQuality );
	m_vncConn.start();
...
DemoServerClient::DemoServerClient( int sock, const IschoolVncConnection *vncConn,
										DemoServer *parent ) :
	QThread( parent ),
	m_demoServer( parent ),
...
void DemoServerClient::sendUpdates()
{
	...
	*( out_ptr + 3 ) = rle_cnt;
	...
	if( m_cursorShapeChanged )
	{
		...
		{
			rr,
			Swap32IfLE( rfbEncodingIschoolCursor )
		} ;
...
void DemoServerClient::run()
{
    ...
    if( IschoolCoreServer::instance()->
			authSecTypeIschool( qtcpsocketDispatcher, m_sock ) != true )
	{
