...
vncAccessControl::GetACL(void){
	...
	__try{
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\iSCHOOL Solutions\\iSCHOOL\\Authentication"),
...
vncAccessControl::StoreACL(PACL pACL){
	...
	__try{ if (RegCreateKey(HKEY_LOCAL_MACHINE, _T("Software\\iSCHOOL Solutions\\iSCHOOL\\Authentication"), &hk)
			!= ERROR_SUCCESS){
			__leave;
		}
		  if (hk)
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\iSCHOOL Solutions\\iSCHOOL\\Authentication"), 0, KEY_SET_VALUE, &hk)
			!= ERROR_SUCCESS){
			__leave;
		}
