...
#ifdef ULTRAVNC_ISCHOOL_SUPPORT
extern BOOL ultravnc_ischool_load_int( LPCSTR valname, LONG *out );
extern BOOL ultravnc_ischool_ask_permission( const char *username, const char *host );
#endif
...
#ifndef ULTRAVNC_ISCHOOL_SUPPORT
...
vncClientThread::CheckLoopBack()
{
	...
	else
	{
		...
		if ((localname != NULL) && (remotename != NULL))
		{
			...
			{
				LONG val = 0;
				if( ultravnc_ischool_load_int( "LocalConnectOnly", &val ) && val )
				{
					// FOOOOOOO
...
BOOL vncClientThread::AuthenticateClient(std::vector<CARD8>& current_auth)
{
	...
	auth_types.push_back(rfbSecTypeIschool);
#endif
	...
	case rfbUltraVNC_SessionSelect:
		...
		break;
#ifdef ULTRAVNC_ISCHOOL_SUPPORT
	case rfbSecTypeIschool:
		auth_success =
			IschoolCoreServer::instance()->
				authSecTypeIschool( vsocketDispatcher, m_socket );
		break;
#endif
	default:
		auth_success = FALSE;
...
BOOL vncClientThread::AuthenticateLegacyClient()
{
	...
	// an old VNC viewer - this allows to use the iSCHOOL client as regular
	// VNC server
	auth_type = rfbLegacy_MsLogon;
#endif
...
vncClientThread::AuthMsLogon(std::string& auth_message) 
{
	...
	if (result) {
#ifdef ULTRAVNC_ISCHOOL_SUPPORT
		return ultravnc_ischool_ask_permission( user, m_socket->GetPeerName() );
#endif
		return TRUE;
	} else {
		return FALSE;
...
//							vnclog.Print(LL_INTINFO, VNCLOG("########mouse_event :%i %i \n"),x,y);
        ...
        case rfbNotifyPluginStreaming:
            ...
            break;
#ifdef ULTRAVNC_ISCHOOL_SUPPORT
		case rfbIschoolCoreRequest:
		{
			omni_mutex_lock l(m_client->GetUpdateLock());
			if( !IschoolCoreServer::instance()->
				handleIschoolClientMessage( vsocketDispatcher, m_socket ) )
			{
				connected = FALSE;
			}
			break;
