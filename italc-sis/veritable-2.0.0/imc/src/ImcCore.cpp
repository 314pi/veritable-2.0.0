/*
 * ImcCore.cpp - global instances for the vERITABLE Management Console
 *
...
{
	criticalMessage( MainWindow::tr( "vERITABLE Management Console" ), msg );
}


bool applyConfiguration( const VeritableConfiguration &c )
{
	// merge configuration
	*VeritableCore::config += c;

	// do neccessary modifications of system configuration
	if( !SystemConfigurationModifier::setServiceAutostart(
									VeritableCore::config->autostartService() ) )
	{
		configApplyError(
			MainWindow::tr( "Could not modify the autostart property "
										"for the vERITABLE Service." ) );
	}

	if( !SystemConfigurationModifier::setServiceArguments(
									VeritableCore::config->serviceArguments() ) )
	{
		configApplyError(
	...
	if( !SystemConfigurationModifier::enableFirewallException(
							VeritableCore::config->isFirewallExceptionEnabled() ) )
	{
		configApplyError(
	...
	localStore.flush( VeritableCore::config );
...
static void listConfiguration( const VeritableConfiguration::DataMap &map,
									const QString &parentKey )
{
	for( VeritableConfiguration::DataMap::ConstIterator it = map.begin();
												it != map.end(); ++it )
	{
