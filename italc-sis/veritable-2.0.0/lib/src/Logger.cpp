	...
	m_logFile( NULL )
{
	...
	int ll = VeritableCore::config->logLevel();
...
#ifdef VERITABLE_BUILD_WIN32
	if( VeritableCore::config->logToWindowsEventLog() )
	{
...
{
	QString logPath = LocalSystem::Path::expand( VeritableCore::config->logFileDirectory() );
...
void Logger::outputMessage( const QString &msg )
{
	...
	if( !VeritableCore::config || VeritableCore::config->logToStdErr() )
	{
