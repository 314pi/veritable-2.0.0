...
bool DesktopAccessPermission::ask( const QString &user, const QString &host )
{
	...
	switch( m_authenticationMethod )
	{
		case KeyAuthentication:
			if( !VeritableCore::config->isPermissionRequiredWithKeyAuthentication() )
			{
				return true;
			}
			break;
		case LogonAuthentication:
			if( !VeritableCore::config->isPermissionRequiredWithLogonAuthentication() )
			{
				return true;
			}
			if( VeritableCore::config->isSameUserConfirmationDisabled() &&
					!user.isEmpty() &&
	...
	int r = VeritableCoreServer::instance()->slaveManager()->
