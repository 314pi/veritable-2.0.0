/*
 * VeritableVncServer.cpp - implementation of VeritableVncServer, a VNC-server-
...
static rfbBool veritableCoreNewClient( struct _rfbClientRec *, void * * )
{
	return true;
}
...
static rfbBool lvs_veritableHandleMessage( struct _rfbClientRec *client,
					void *data,
					const rfbClientToServerMsg *message )
{
	if( message->type == rfbVeritableCoreRequest )
	{
		return VeritableCoreServer::instance()->
				handleVeritableClientMessage( libvncServerDispatcher, client );
	}
	return false;
}
...
static void lvs_veritableSecurityHandler( struct _rfbClientRec *cl )
{
	bool authOK = VeritableCoreServer::instance()->
								authSecTypeVeritable( libvncServerDispatcher, cl );
...
VeritableVncServer::VeritableVncServer() :
	QThread(),
	m_port( VeritableCore::serverPort )
{
}
...
static void runX11vnc( QStringList cmdline, int port, bool plainVnc )
{
	...
	if( !plainVnc && VeritableCore::config->isHttpServerEnabled() )
	{
		QDir d( QCoreApplication::applicationDirPath() );
		if( d.cdUp() && d.cd( "share" ) && d.cd( "veritable" ) &&
				d.cd( "JavaViewer" ) )
		{
			cmdline << "-httpport"
					<< QString::number( VeritableCore::config->httpServerPort() )
		...
		else
		{
			qWarning( "Could not find JavaViewer files. "
						"Check your vERITABLE installation!" );
		}
	}
	...
	if( plainVnc == false )
	{
		// register vERITABLE protocol extension
		rfbProtocolExtension pe;
		pe.newClient = veritableCoreNewClient;
		pe.init = NULL;
		pe.enablePseudoEncoding = NULL;
		pe.pseudoEncodings = NULL;
		pe.handleMessage = lvs_veritableHandleMessage;
		pe.close = NULL;
		pe.usage = NULL;
	...
	// register handler for vERITABLE's security-type
	rfbSecurityHandler shi = { rfbSecTypeVeritable, lvs_veritableSecurityHandler, NULL };
...
void VeritableVncServer::runVncReflector( int srcPort, int dstPort )
{
#ifdef VERITABLE_BUILD_WIN32
	...
	if( VeritableCore::config->isDemoServerMultithreaded() )
	{
		args << "-threads";
	}
