...
#ifdef VERITABLE_HAVE_PWD_H
#include <pwd.h>
#endif

#ifdef VERITABLE_HAVE_SYS_SOCKET_H
#include <sys/socket.h>
#endif

#ifdef VERITABLE_HAVE_ARPA_INET_H
#include <arpa/inet.h>
#endif

#ifdef VERITABLE_HAVE_NETINET_IN_H
#include <netinet/in.h>
#endif

#ifdef VERITABLE_HAVE_ERRNO_H
#include <errno.h>
#endif
...
#ifdef VERITABLE_BUILD_WIN32
		LocalSystem::windowsConfigPath( CSIDL_APPDATA ) + QDir::separator() + "vERITABLE";
#else
		QDir::homePath() + QDir::separator() + ".veritable";
#endif
...
QString Path::privateKeyPath( VeritableCore::UserRoles role, QString baseDir )
{
	if( baseDir.isEmpty() )
	{
		baseDir = expand( VeritableCore::config->privateKeyBaseDir() );
	}
	else
	{
		baseDir += "/private";
	}
	QString d = baseDir + QDir::separator() + VeritableCore::userRoleName( role ) +
					QDir::separator() + "key";
	return QDTNS( d );
}
...
QString Path::publicKeyPath( VeritableCore::UserRoles role, QString baseDir )
{
	if( baseDir.isEmpty() )
	{
		baseDir = expand( VeritableCore::config->publicKeyBaseDir() );
	}
	else
	{
		baseDir += "/public";
...
{
#ifdef VERITABLE_BUILD_WIN32
	return QDTNS( windowsConfigPath( CSIDL_COMMON_APPDATA ) + QDir::separator() + "vERITABLE/" );
#else
	return "/etc/veritable/";
#endif
}
