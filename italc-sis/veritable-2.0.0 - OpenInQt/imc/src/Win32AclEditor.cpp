...
STDMETHODIMP VeritableSecurityInfo::QueryInterface( REFIID iid, void** ppv )
{
...
STDMETHODIMP_(ULONG) VeritableSecurityInfo::AddRef()
{
	return ++m_cRefs;
}
STDMETHODIMP_(ULONG) VeritableSecurityInfo::Release()
{
	ULONG n = --m_cRefs;
	if ( 0 == n )
		delete this;
...
STDMETHODIMP
VeritableSecurityInfo::GetObjectInformation( SI_OBJECT_INFO* poi ){
...
STDMETHODIMP
VeritableSecurityInfo::GetSecurity(SECURITY_INFORMATION ri,
...
STDMETHODIMP
VeritableSecurityInfo::SetSecurity(SECURITY_INFORMATION ri, void* psd){
	vncAccessControl vncAC;
	return vncAC.SetSD((PSECURITY_DESCRIPTOR) psd) ? S_OK : E_FAIL;
}

STDMETHODIMP
VeritableSecurityInfo::PropertySheetPageCallback(HWND hwnd, UINT msg, SI_PAGE_TYPE pt){
...
STDMETHODIMP
VeritableSecurityInfo::GetAccessRights(const GUID*,
								 DWORD dwFlags,
								 SI_ACCESS** ppAccess,
								 ULONG* pcAccesses,
...
STDMETHODIMP
VeritableSecurityInfo::MapGeneric(const GUID*, UCHAR* pAceFlags, ACCESS_MASK* pMask){
...
STDMETHODIMP
VeritableSecurityInfo::GetInheritTypes(SI_INHERIT_TYPE** ppInheritTypes, ULONG* pcInheritTypes){
...
void Win32AclEditor( HWND hwnd )
{
	...
	VeritableSecurityInfo* psi = new VeritableSecurityInfo(L"vERITABLE Server", L"vERITABLE Server");
	psi->AddRef();
	...
	psh.hwndParent	= hwnd;
	psh.pszCaption	= _T("vERITABLE Logon Authentication");
