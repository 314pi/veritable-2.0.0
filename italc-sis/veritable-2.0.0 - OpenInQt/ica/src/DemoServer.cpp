	...
	m_vncConn()
{
	...
	m_vncConn.setVeritableAuthType( VeritableAuthCommonSecret );
	m_vncConn.setQuality( VeritableVncConnection::DemoServerQuality );
	m_vncConn.start();
...
DemoServerClient::DemoServerClient( int sock, const VeritableVncConnection *vncConn,
										DemoServer *parent ) :
	QThread( parent ),
	m_demoServer( parent ),
...
void DemoServerClient::sendUpdates()
{
	...
	*( out_ptr + 3 ) = rle_cnt;
	...
	if( m_cursorShapeChanged )
	{
		...
		{
			rr,
			Swap32IfLE( rfbEncodingVeritableCursor )
		} ;
...
void DemoServerClient::run()
{
    ...
    if( VeritableCoreServer::instance()->
			authSecTypeVeritable( qtcpsocketDispatcher, m_sock ) != true )
	{
