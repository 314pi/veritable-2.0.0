	...
	m_sysKeyTrapper( new SystemKeyTrapper( false ) )
{
	...
	if( m_mode == DemoMode )
	{
		m_vncConn.setQuality( VeritableVncConnection::DemoClientQuality );
		m_vncConn.setVeritableAuthType( VeritableAuthHostBased );
	...
	{
		m_vncConn.setQuality( VeritableVncConnection::RemoteControlQuality );
	}
