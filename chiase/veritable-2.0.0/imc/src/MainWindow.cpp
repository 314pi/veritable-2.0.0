...
#define CONNECT_BUTTON_SLOT(name) \
	...
	connect( VeritableCore::config, SIGNAL( configurationChanged() ),
				this, SLOT( configurationChanged() ) );

#ifndef VERITABLE_BUILD_WIN32
...
void MainWindow::reset( bool onlyUI )
{
	if( onlyUI == false )
	{
		VeritableCore::config->clear();
		*VeritableCore::config += VeritableConfiguration::defaultConfiguration();
		*VeritableCore::config += VeritableConfiguration( Configuration::Store::LocalBackend );
	}

#ifdef VERITABLE_BUILD_WIN32
	// always make sure we do not have a LogonACL string in our config
	VeritableCore::config->removeValue( "LogonACL", "Authentication" );

	// revert LogonACL to what has been saved in the encoded logon ACL
	LogonAclSettings().setACL(
		VeritableCore::config->value( "EncodedLogonACL", "Authentication" ) );
#endif

	FOREACH_VERITABLE_CONFIG_PROPERTY(INIT_WIDGET_FROM_PROPERTY)
...
{
#ifdef VERITABLE_BUILD_WIN32
	VeritableCore::config->setValue( "EncodedLogonACL", LogonAclSettings().acl(),
															"Authentication" );
#endif
	if( ImcCore::applyConfiguration( *VeritableCore::config ) )
	{
#ifdef VERITABLE_BUILD_WIN32
		if( isServiceRunning() &&
			QMessageBox::question( this, tr( "Restart vERITABLE Service" ),
				tr( "All settings were saved successfully. In order to take "
					"effect the vERITABLE service needs to be restarted. "
...
{
	serviceControlWithProgressBar( tr( "Starting vERITABLE service" ), "-startservice" );
}
...
{
	serviceControlWithProgressBar( tr( "Stopping vERITABLE service" ), "-stopservice" );
}
...
void MainWindow::clearLogFiles()
{
	...
	if( isServiceRunning() )
	{
		if( QMessageBox::question( this, tr( "vERITABLE Service" ),
				tr( "The vERITABLE service needs to be stopped temporarily "
	...
	bool success = true;
	QDir d( LocalSystem::Path::expand( VeritableCore::config->logFileDirectory() ) );
	foreach( const QString &f, d.entryList( QStringList() << "Veritable*.log" ) )
	{
		if( f != "VeritableManagementConsole.log" )
		{
			success &= d.remove( f );
		}
	}
...
void MainWindow::loadSettingsFromFile()
{
	...
	if( !fileName.isEmpty() )
	{
		...
		Configuration::XmlStore( Configuration::XmlStore::System,
										fileName ).load( VeritableCore::config );
		reset( true );
...
#ifdef VERITABLE_BUILD_WIN32
		VeritableCore::config->removeValue( "LogonACL", "Authentication" );
		VeritableCore::config->setValue( "EncodedLogonACL",
								LogonAclSettings().acl(), "Authentication" );
#endif

		// write current configuration to output file
		Configuration::XmlStore( Configuration::XmlStore::System,
										fileName ).flush( VeritableCore::config );
...
void MainWindow::generateBugReportArchive()
{
	...
	QString outfile = fsb.exec( QDir::homePath(),
								...
								tr( "vERITABLE bug report archive (*.ibra.xml)" ) );
	if( outfile.isEmpty() )
	{
		return;
	}
...
#elif defined( VERITABLE_BUILD_LINUX )
...
#ifdef VERITABLE_HOST_X86
	const QString buildType = "x86";
#elif defined( VERITABLE_HOST_X86_64 )
	...
	obj.setValue( "Version", VERITABLE_VERSION, "General" );


	// add current vERITABLE configuration
	obj.addSubObject( VeritableCore::config, "Configuration" );


	// compress all log files and encode them as base64
	QStringList paths;
	paths << LocalSystem::Path::expand( VeritableCore::config->logFileDirectory() );
#ifdef VERITABLE_BUILD_WIN32
	...
	QMessageBox::information( this, tr( "vERITABLE bug report archive saved" ),
			tr( "An vERITABLE bug report archive has been saved to %1. "
				"It includes vERITABLE log files and information about your "
...
void MainWindow::aboutVeritable()
{
