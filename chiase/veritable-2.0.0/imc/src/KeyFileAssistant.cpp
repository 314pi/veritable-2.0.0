	...
	m_ui( new Ui::KeyFileAssistant )
{
	...
	QDir d( LocalSystem::Path::expand( VeritableCore::config->privateKeyBaseDir() ) );
	d.cdUp();
...
{
	VeritableCore::UserRole role =
			static_cast<VeritableCore::UserRole>(
					m_ui->userRole->currentIndex() + VeritableCore::RoleTeacher );
