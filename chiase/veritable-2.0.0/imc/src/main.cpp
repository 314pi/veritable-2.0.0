/*
 * main.cpp - main file for vERITABLE Management Console
 *
...
int main( int argc, char **argv )
{
	...
	Logger l( "VeritableManagementConsole" );

	if( !VeritableConfiguration().isStoreWritable() &&
			VeritableCore::config->logLevel() < Logger::LogLevelDebug )
	{
		ImcCore::criticalMessage( MainWindow::tr( "Configuration not writable" ),
			MainWindow::tr( "The local configuration backend reported that the "
							"configuration is not writable! Please run the vERITABLE "
	...
	while( argc > 1 && argIt.hasNext() )
	{
		...
		if( ( a == "-applysettings" || a == "-a" ) && argIt.hasNext() )
		{
			...
			if( ImcCore::applyConfiguration( VeritableConfiguration( &xs ) ) )
			{
				ImcCore::informationMessage(
					MainWindow::tr( "vERITABLE Management Console" ),
		...
		{
			ImcCore::listConfiguration( *VeritableCore::config );
		...
		else if( a == "-setconfigvalue" || a == "-s" )
		{
			...
			VeritableCore::config->setValue( key, value, parentKey );

			ImcCore::applyConfiguration( *VeritableCore::config );
		...
		else if( a == "-createkeypair" )
		{
			...
			ImcCore::createKeyPair( VeritableCore::role, destDir );
			return 0;
		}
		else if( a == "-importpublickey" || a == "-i" )
		{
			...
			if( !ImcCore::importPublicKey( VeritableCore::role, pubKeyFile, QString() ) )
			{
