...
class VeritableServiceSubProcess
{
public:
	VeritableServiceSubProcess() :
...
bool WindowsService::evalArgs( int &argc, char **argv )
{
	...
	{
		VeritableCore::init();
		Logger l( "VeritableServiceMonitor" );
		return runAsService();
	}
	...
	Logger l( "VeritableServiceControl" );
...
{
	VeritableServiceSubProcess veritableProcess;
	...
	while( WaitForSingleObject( s_stopServiceEvent, 1000 ) == WAIT_TIMEOUT )
	{
		...
		if( oldSessionId != sessionId || sessionChanged )
		{
			...
			if( oldSessionId != SESSION_INVALID || sessionChanged )
			{
				...
				while( lastServiceStart.elapsed() < 10000 && veritableProcess.isRunning() );

				veritableProcess.stop();

				Sleep( 5000 );
			}
			if( sessionId != SESSION_INVALID || sessionChanged )
			{
				veritableProcess.start( sessionId );
				lastServiceStart.restart();
			}

			oldSessionId = sessionId;
		}
		else if( veritableProcess.isRunning() == false )
		{
			veritableProcess.start( sessionId );
			oldSessionId = sessionId;
			lastServiceStart.restart();
